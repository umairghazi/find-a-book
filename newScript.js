...
function createMenu(root, level, selectedVal) {
    ...
    if (root.options) {   //as long as options are there in the data, do this..
    ...
    if (root.options === null) {  //if the end of data chain is reached, do this..
        var x = document.getElementsByTagName("select");
        var opVals = "";
        for (var i = 0; i < x.length; i++) {  //get values from all the selected options to show final selection

            if (BrowserDetect.browser === "Mozilla" || BrowserDetect.browser === "Explorer") { //because selectedOptions doesn't work with IE
...
function handleHttpResponse() {
    if (http.readyState === 4) {
        if (http.status === 200) {
            ...
            var bookTitle = xmlDocument.getElementsByTagName("original_title")[0].textContent;
            var bookAuthor = xmlDocument.getElementsByTagName("name")[0].textContent;
            var averageRating = xmlDocument.getElementsByTagName("average_rating")[0].textContent;
            var bookImageURL = xmlDocument.getElementsByTagName("image_url")[0].textContent;
            var publicationYear = xmlDocument.getElementsByTagName("original_publication_year")[0].textContent;
            var description = xmlDocument.getElementsByTagName("description")[0].textContent;
            ...
            displayRating.innerHTML = "Average rating on Goodreads: " + averageRating + "<br/>";
...
function getYouTube(url) {
    if (url) {
        ...
        videoTitle.innerHTML = "There is a movie based on this novel you might be interested in";
